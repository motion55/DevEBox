...
void testFilledRoundRects()
{
	...
	for (i = min(_width, _height); i > 20; i -= 6)
	{
		i2 = i / 2;
		green -= step;
		Displ_fillRoundRect(cx - i2, cy - i2, i, i, i / 8,
				color565(0, green, 0));
	}
}
...
void Displ_ColorTest()
{
	...
	{ DD_WHITE, RED, BLUE, GREEN, YELLOW, MAGENTA, CYAN, WHITE,
	DDD_WHITE, D_RED, D_BLUE, D_GREEN, D_YELLOW, D_MAGENTA, D_CYAN, D_WHITE,
	DDDD_WHITE, DD_RED, DD_BLUE, DD_GREEN, DD_YELLOW, DD_MAGENTA, DD_CYAN,
			DD_WHITE };
...
void Displ_PerfTest()
{
	...
	for (uint8_t k1 = 0; k1 < 4; k1++)
	{
		...
		for (uint8_t k = 1; k < 5; k++)
		{
			...
			Displ_CString(0, 10 + deltah + (k) * Font20.Height * 2, _width,
					10 + deltah + Font20.Height * ((k) * 2 + 1), riga, Font20,
					1, WHITE, BGCOLOR);
		};
...
#ifdef DISPLAY_DIMMING_MODE
		{
			...
			while (currlevel > Displ_BackLight('+'))
			{
			};//restoring previous backlight level: now, increase light until it is set to the "currlevel" stored above
		}
#else
		HAL_Delay(5000);
#endif
