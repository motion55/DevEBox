...
#endif // Z_RGB666
#else
uint8_t dispBuffer1[SIZEBUF] DISPLAY_BUFFER_SECTION;
uint8_t dispBuffer2[SIZEBUF] DISPLAY_BUFFER_SECTION;
...
//if display color format is RGB666: convert RGB565 received by TouchGFX and swap bytes
		...
		for (uint16_t *data16 = (uint16_t*) data;
				(data16 < limit) & ((buf8Pos - dispBuffer1) < (SIZEBUF - 3));
				data16++)
		{
...
#if (defined(DISPL_RST_GPIO_Port)&&defined(DISPL_RST_Pin))
...
#elif defined(DISPL_CS_Pin)
	HAL_GPIO_WritePin(DISPL_CS_GPIO_Port, DISPL_CS_Pin, GPIO_PIN_RESET); // select display
#endif
	ILI9XXX_Init();
	Displ_Orientation(orientation);

#if defined(DISPLAY_DIMMING_MODE)
	Displ_BackLight('1');
#else
...
#if (defined(EXT_FLASH_BASEADDRESS)&&defined(EXT_FLASH_SIZE))
	if (((uint32_t) data >= EXT_FLASH_BASEADDRESS)
			&& ((uint32_t) data < EXT_FLASH_BASEADDRESS + EXT_FLASH_SIZE))
	{
		...
		while (size > SIZEBUF)
		{
			...
			size -= SIZEBUF;
			dispBuffer =
					(dispBuffer == dispBuffer1 ? dispBuffer2 : dispBuffer1); // swapping buffer
		}
